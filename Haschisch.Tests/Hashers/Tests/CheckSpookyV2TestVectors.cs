...
{
    public class CheckSpookyV2TestVectors
    {
        private static readonly uint[] ExpectedU32HashCodes = new uint[]
        {
        ...
        [Test]
        public void Hash_SpookyV2_HashBytes_ReturnsExpectedHashCode()
        {
            var buf = new byte[ExpectedU32HashCodes.Length];
            var actual = new uint[buf.Length];
            for (int i = 0; i < buf.Length; i++)
            {
                buf[i] = (byte)(i + 128);
                actual[i] = (uint)default(SpookyV2Hasher.Block).Hash((0, 0), buf, 0, i).Item1;
                Assert.AreEqual(ExpectedU32HashCodes[i], actual[i], "TestVector #{0} failed, expected 0x{1:X}, got 0x{2:X}", i, ExpectedU32HashCodes[i], actual[i]);
            }
        }
    }
}
