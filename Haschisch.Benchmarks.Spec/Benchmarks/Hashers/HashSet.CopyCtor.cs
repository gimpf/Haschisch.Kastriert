...
namespace Haschisch.Benchmarks
{
    public class HashSet_CopyCtor
    {
        ...
        {
            var src = new HashSet<Large>(this.data, comparer);
            // this changes iteration order, and in effect, behavior with conflicting hash-codes
            // so, relative performance across algorithms _might_ change compared to regular fill
            var dst = new HashSet<Large>(src, comparer);
            return dst;
        }
    }
}
