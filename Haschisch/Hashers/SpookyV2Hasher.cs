...
namespace Haschisch.Hashers
{
    ...
    public static class SpookyV2Hasher
    {
        ...
        public struct Combiner : IHashCodeCombiner
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public int Combine<T1, T2, T3, T4, T5, T6>(T1 x1, T2 x2, T3 x3, T4 x4, T5 x5, T6 x6)
            {
                var h0 = x1?.GetHashCode() ?? 0;
                var h1 = x2?.GetHashCode() ?? 0;
                var h2 = x3?.GetHashCode() ?? 0;
                ...
                return (int)SpookyV2Steps.Short.Finish(
                    ref s0, ref s1, ref s2, ref s3,
                    (uint)h0 | (ulong)h1 << 32, (uint)h2 | (ulong)h3 << 32, (uint)h4 | (ulong)h5 << 32, 0,
                    6 * sizeof(int)).Item1;
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public int Combine<T1, T2, T3, T4, T5, T6, T7>(T1 x1, T2 x2, T3 x3, T4 x4, T5 x5, T6 x6, T7 x7)
            {
                var h0 = x1?.GetHashCode() ?? 0;
                var h1 = x2?.GetHashCode() ?? 0;
                var h2 = x3?.GetHashCode() ?? 0;
                ...
                return (int)SpookyV2Steps.Short.Finish(
                    ref s0, ref s1, ref s2, ref s3,
                    (uint)h0 | (ulong)h1 << 32, (uint)h2 | (ulong)h3 << 32, (uint)h4 | (ulong)h5 << 32, (uint)h6,
                    7 * sizeof(int)).Item1;
            }
